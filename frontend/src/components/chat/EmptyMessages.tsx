...
export default function EmptyMessages({ onSendClick }: Props) {
  return (
    <div className="flex flex-col items-center justify-center text-center flex-1 py-8 px-7 m-5 bg-white rounded-[20px] shadow-[0_4px_20px_rgba(37,99,235,0.06)] dark:bg-[var(--chat-panel-bg)] dark:shadow-none">
      <div className="relative w-40 h-[110px] mb-6">
        ...
        <div className="absolute flex items-center justify-center gap-1 rounded-2xl bottom-0 right-3 w-24 h-[38px] bg-[#2563eb] rounded-br-[4px]">
      ...
      {onSendClick && (
        <button
          className="border-none bg-[#2563eb] text-white font-semibold text-[15px] py-3 px-7 rounded-full cursor-pointer dark:bg-[var(--chat-accent)]"
          onClick={onSendClick}
          type="button"
        >
          Send a message
